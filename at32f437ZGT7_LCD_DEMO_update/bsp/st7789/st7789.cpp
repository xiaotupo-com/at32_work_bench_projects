 ...
 * @param res_pin 
 ********************************************************************************/
St7789::St7789(gpio_type *cs_gpio, uint16_t cs_pin, gpio_type *dc_gpio,
               uint16_t dc_pin, gpio_type *res_gpio, uint16_t res_pin)
    : m_cs_gpio(cs_gpio, cs_pin), m_dc_gpio(dc_gpio, dc_pin),
...
{
    setBacklight(OnOffState::off);// 先关闭背光
    uint8_t _data;
    setCS(HighLowState::high);
    reset();
 ...
 * @brief 背光控制
 * 
 * @param state BlkState::0ff 关闭背光，BlkState::on 开启背光
 ********************************************************************************/
void St7789::setBacklight(OnOffState state) const
{
#if defined(AT32)
 ...
 * @brief 片选选择
 * 
 * @param state HighLowState::high 释放，HighLowState::low 片选选择
 ********************************************************************************/
void St7789::setCS(HighLowState state) const
{
    if (state == HighLowState::high) {
        m_cs_gpio.setLevel(TRUE);
    }

    if (state == HighLowState::low) {
...
{
    setCS(HighLowState::low);// cs 片选
    setDC(HighLowState::low);// 写命令
    // 如果 发送缓存区不为空，则一直循环
...
{
    setCS(HighLowState::low); // cs 片选
    setDC(HighLowState::high);// 写数据

#if defined(USE_DMA)
    /* config dma channel transfer parameter */
    /* user need to modify define values DMAx_CHANNELy_XXX_BASE_ADDR and
   * DMAx_CHANNELy_BUFFER_SIZE in at32xxx_wk_config.h */
    dma_channel_enable(DMA1_CHANNEL1, FALSE);
    wk_dma_channel_config(DMA1_CHANNEL1, (uint32_t) &SPI1->dt, (uint32_t) data,
                          size);
...
#endif                        // USE_DMA
    setCS(HighLowState::high);// cs 释放// cs 释放
}
 ...
 * @param y2 
 ********************************************************************************/
void St7789::setLcdAddress(uint16_t x1, uint16_t y1, uint16_t x2,
                           uint16_t y2) const
{
 ...
 * @param color 
 ********************************************************************************/
void St7789::drawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
                      uint16_t color)
{
    uint16_t swap;
 ...
 * @param color 
 ********************************************************************************/
void St7789::drawRectangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                           uint16_t color)
{
 ...
 * @param bgcolor 
 ********************************************************************************/
void St7789::writeChar(uint16_t x, uint16_t y, char ch, FontDef font,
                       uint16_t color, uint16_t bgcolor)
{
    uint32_t i, b, j;
    setCS(HighLowState::low);
 ...
 * @param bgcolor 
 ********************************************************************************/
void St7789::writeString(uint16_t x, uint16_t y, const char *str, FontDef font,
                         uint16_t color, uint16_t bgcolor)
{
    setCS(HighLowState::low);
    while (*str) {
 ...
 * @param bgcolor 
 ********************************************************************************/
void St7789::writeNum(uint16_t x, uint16_t y, uint32_t num, const uint8_t len,
 ...
 * @param bgcolor 
 ********************************************************************************/
void St7789::writeFloat(uint16_t x, uint16_t y, float num, const uint8_t len,
                        FontDef font, uint16_t color, uint16_t bgcolor)
{
    char str[len];
    sprintf(str, "%.2f", num);
 ...
 * @brief 数据命令选择
 * 
 * @param state HighLowState::high   发送数据，HighLowState::low    发送命令
 ********************************************************************************/
void St7789::setDC(HighLowState state) const
{
    if (state == HighLowState::high) {
